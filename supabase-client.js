...
Object.assign(DB, {
  ...
  async getLembretes(){
    ...
    return data.map(function(r){
      return { id:r.id, texto:r.texto, dataInicio:r.data_inicio, diasTotal:r.dias_total!=null?Number(r.dias_total):null, dataHora:r.data_hora, horario:r.horario||null, ativo:r.ativo };
    });
  },
  async upsertLembrete(reg){
    const row = { id:(reg.id&&reg.id.length===36)?reg.id:undefined, texto:reg.texto, data_inicio:reg.dataInicio,
      dias_total:reg.diasTotal!=null?reg.diasTotal:null, data_hora:reg.dataHora||null, horario:reg.horario||null, ativo:reg.ativo!==false };
    ...
    return { id:data.id, texto:data.texto, dataInicio:data.data_inicio, diasTotal:data.dias_total!=null?Number(data.dias_total):null, dataHora:data.data_hora, horario:data.horario||null, ativo:data.ativo };
  },
